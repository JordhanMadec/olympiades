...
import { matchesService, gamesService, teamsService, drawsService } from '../../services';
import { Match, Game, Team, MatchStatus, UpdateScoresDto, GameFormat, RoundRobinMatch, BracketMatch } from '../../types';
...
export function MatchesSettings() {
  ...
  const handleGenerateBracket = async () => {
    ...
    try {
      setGenerating(true);
      const draws = await drawsService.generateBracket({ gameId: drawGameId, teamIds: selectedTeamIds, useSeeding: false });
  ...
  const handleCreateMatches = async () => {
    ...
    if (draws.length === 0) { alert('Générez d\'abord un tirage'); return; }
  ...
  return (
    <div>
      ...
      {/* Draw panel */}
      {showDrawPanel && (
        <div className="bg-surface-200 border border-surface-border rounded-2xl p-6 mb-6">
      ...
      {matches.length === 0 ? (
        ...
        <div className="space-y-3">
          {matches.map((match) => (
            <div key={match.id} className="bg-surface-200 border border-surface-border rounded-xl p-5">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3">
                  <span className="text-white font-medium text-sm">{getGameName(match.gameId)} — Match #{match.matchNumber}</span>
              ...
              <div className="flex flex-wrap gap-2">
                {match.matchTeams.sort((a, b) => (a.rank || 999) - (b.rank || 999)).map((mt) => (
                  <div
                    key={mt.id}
                    className={`flex items-center gap-2 rounded-lg px-2.5 py-1.5 border text-xs ${
                      mt.rank === 1 ? 'bg-primary-500/10 border-primary-500/30' : 'bg-surface-400 border-surface-border'
                    }`}
                  >
                    ...
                    {mt.rawScore != null && (
                      <span className="text-primary-400 font-bold">{mt.rawScore}</span>
                    )}
                  </div>
                ))}
              </div>
        ...
        >
          <form onSubmit={handleSubmitScores}>
            <div className="space-y-3 mb-6">
              {scoringMatch.matchTeams.map((mt) => (
                <div key={mt.teamId}>
                  ...
                  />
                  {mt.rank && (
                    <p className="text-xs text-zinc-500 -mt-3">Rang: {mt.rank} — Points: {mt.points}</p>
                  )}
                </div>
              ))}
            </div>
            <div className="flex gap-3">
              ...
              <Button type="button" variant="secondary" onClick={() => setScoringMatch(null)} className="flex-1" disabled={saving}>
