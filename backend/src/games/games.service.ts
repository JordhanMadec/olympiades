...
export class GamesService {
  ...
  private validateGameRules(
    gameData: Partial<CreateGameDto | UpdateGameDto>,
  ): void {
    ...
    if (gameType === GameType.SCORE) {
      if (gameFormat && gameFormat !== GameFormat.ELIMINATION) {
        throw new BadRequestException(
          "SCORE games must use ELIMINATION format",
        );
      }
      if (teamsPerMatch && teamsPerMatch !== 2) {
        throw new BadRequestException(
          "SCORE games must have exactly 2 teams per match",
        );
      }
    }
    ...
    if (gameType === GameType.TIME || gameType === GameType.POINTS) {
      if (gameFormat && gameFormat !== GameFormat.ROUND_ROBIN) {
        throw new BadRequestException(
          "TIME and POINTS games must use ROUND_ROBIN format",
        );
      }
    }
  }
